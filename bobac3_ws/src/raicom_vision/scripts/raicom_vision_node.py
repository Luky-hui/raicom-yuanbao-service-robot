...
class RaicomVisionNode:
    def __init__(self):
        ...
        rospy.loginfo(
            "RAICOM vision ready: model=%s classes=%d templates=%d service=%s",
            model_path,
    ...
    def detect(self, request):
        ...
        unknown_labels = [
            ...
            if label not in self.class_ids and label not in self.template_images
        ]
        if unknown_labels:
        ...
        image_topic = request.image_topic or self.config["default_image_topic"]
        ...
        try:
            ...
            response.success = False
            response.message = "Image unavailable on %s: %s" % (image_topic, exc)
            return response

        yolo_labels = [label for label in requested_labels if label in self.class_ids]
        template_labels = [
            label for label in requested_labels if label in self.template_images
        ]
        started = time.time()
        detections = []
        detected_labels = set()
    ...
    @staticmethod
    def append_detection(response, detections, label, score, coordinates):
